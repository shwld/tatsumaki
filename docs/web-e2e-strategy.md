...
- エージェント（開発時）
  ...
  - CI の失敗 artifact を確認し、原因を修正する
  - 意図した UI 変更の場合のみ Linux のスクリーンショットベースラインを更新する
- CI（検証時）
  - Linux で既存テストを決定論的設定で実行する
  - 成否判定のみを担当し、テスト生成や仕様判断は行わない

### 手順

1. エージェントがテストコードを更新する
2. Linux CI の `ui-screenshot-diff` を実行し、失敗時は artifact の actual / expected / diff を確認する
3. 意図した UI 変更なら CI artifact の actual 画像を確認して Linux ベースラインへ反映し、CI を再実行する。ローカルでの画像生成・比較は運用対象外とする
