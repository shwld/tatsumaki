...
import { Bell, Tornado } from "lucide-react";
import { useTranslation } from "react-i18next";
import { Link, useLocation } from "react-router";
import { HeaderToolbarContext } from "./header-toolbar";
...
export function Layout({ children }: LayoutProps) {
  ...
  const { hidden, onScrollCapture, reveal } = useScrollHeader({
    headerRef,
    pinned: notificationOpen || accountOpen,
    routeKey: pathname,
  });
  ...
  return (
    <HeaderToolbarContext.Provider value={toolbarHost}>
      <div
        onScrollCapture={onScrollCapture}
        onFocusCapture={(event) => {
          if (headerRef.current?.contains(event.target)) reveal();
        }}
        className="flex h-dvh min-h-dvh flex-col bg-[var(--color-bg)] text-[var(--color-text)] transition-colors"
      >
        <div
          data-testid="app-header-container"
          data-hidden={hidden}
          className={`grid shrink-0 transition-[grid-template-rows] duration-200 motion-reduce:transition-none ${hidden ? "grid-rows-[0fr]" : "grid-rows-[1fr]"}`}
        >
          <div className="min-h-0 overflow-hidden">
            <header
              ...
              className={`border-b border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2 transition-transform duration-200 motion-reduce:transition-none ${hidden ? "-translate-y-full" : "translate-y-0"}`}
            >
              <div className="flex min-w-0 items-center gap-3">
                <Link
                  to="/projects"
                  className="inline-flex size-8 shrink-0 items-center justify-center rounded-md text-[var(--color-text)] hover:bg-[var(--color-hover)]"
                  aria-label={t("app.name")}
                >
                  <Tornado className="size-5" aria-hidden="true" />
                </Link>
                <div ref={setToolbarHost} className="min-w-0 flex-1" />
                {!isLoading && user ? (
